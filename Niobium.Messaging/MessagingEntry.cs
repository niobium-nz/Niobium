        ...
        public T Value
        {
            get
            {
                if (value == null)
                {
                    if (Type == null)
                    {
                        value = JsonSerializer.Deserialize<T>(Body, SerializationOptions)!;
                    }
                    else
                    {
                        Type? type = System.Type.GetType(Type);
                        value = (T)JsonSerializer.Deserialize(Body, type!, SerializationOptions)!;
                    }
                }
            ...
            set
            {
                ...
                {
                    Body = JsonSerializer.Serialize(value, SerializationOptions);
