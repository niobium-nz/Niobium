...
namespace Cod.Platform.Finance.WechatPay
{
    internal class WechatPaymentProcessor : IPaymentProcessor
    {
        ...
        public async Task<OperationResult<ChargeResponse>> ChargeAsync(ChargeRequest request)
        {
            ...
            string apiUri = await configuration.Value.GetSettingAsync<string>(Constants.API_URL);
            ...
            OperationResult<string> prepayid = await wechatIntegration.Value.JSAPIPay(
                payer.OpenID,
                request.Amount,
                payer.AppID,
                request.Source,
