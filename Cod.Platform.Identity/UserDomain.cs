...
{
    public class UserDomain : AccountableDomain<User>, IImpedable, IDomain<User>
    {
        ...
        public UserDomain(
            ...
            IEnumerable<IEventHandler<UserDomain>> eventHandlers,
            ...
            ILogger logger)
            : base(repository, eventHandlers, transactionRepo, accountingRepo, auditors, cache, logger)
        {
        ...
        public async Task<OperationResult<LoginResult>> LoginAsync(OpenIDKind kind, string appID, string authCode)
        {
            ...
            if (login == null)
            {
                string key = await configuration.Value.GetSettingAsync<string>(Constants.AUTH_SECRET_NAME);
        ...
        public async Task<string> IssueTokenAsync(IEnumerable<KeyValuePair<string, string>> entitlements)
        {
            ...
            List<Entitlement> records = await entitlementRepository.Value.GetAsync(Entitlement.BuildPartitionKey(userID)).ToListAsync();
                    ...
                    if (userID.HasValue && login.User != userID.Value)
                    {
                        ...
                        if (openIDs != null)
                        {
                            foreach (OpenID openid in openIDs)
                            {
                                ...
                                if (openid.Identity == registration.Identity
                                    && !openIDsToRemove.Any(o => o.PartitionKey == openid.PartitionKey && o.RowKey == openid.RowKey))
                                {
