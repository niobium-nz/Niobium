        ...
        where T : IEntity
    {
        ...
        public IAsyncEnumerable<Transaction> GetTransactionsAsync(DateTimeOffset fromInclusive, DateTimeOffset toInclusive)
        {
            ...
            return transactionRepo.Value.QueryAsync(
                      Cod.Model.Transaction.BuildPartitionKey(principal),
                      Cod.Model.Transaction.BuildRowKey(toInclusive),
                      Cod.Model.Transaction.BuildRowKey(fromInclusive));
        }
        ...
        public async Task<IEnumerable<Transaction>> MakeTransactionAsync(IEnumerable<TransactionRequest> requests)
        {
            ...
            foreach (TransactionRequest request in requests)
            {
                ...
                request.ID ??= Cod.Model.Transaction.BuildRowKey(DateTimeOffset.UtcNow);
        ...
        public async Task<Transaction> GetTransactionAsync(DateTimeOffset id)
        {
            ...
            Transaction transaction = await transactionRepo.Value.RetrieveAsync(target, Cod.Model.Transaction.BuildRowKey(id));
            return transaction;
        }
        ...
        public async Task<AccountBalance> GetBalanceAsync(DateTimeOffset input)
        {
            ...
            double balance;
            string principal = AccountingPrincipal;
            Accounting accounting = await accountingRepo.Value.RetrieveAsync(Cod.Model.Accounting.BuildPartitionKey(principal), Cod.Model.Accounting.BuildRowKey(lastAccountDate));
            if (accounting == null)
            {
        ...
        private async Task<Accounting> GetLatestAccountingAsync(DateTimeOffset input)
        {
            ...
            List<Accounting> accountings = await accountingRepo.Value.QueryAsync(
                                    Cod.Model.Accounting.BuildPartitionKey(principal),
                                    Cod.Model.Accounting.BuildRowKey(input),
                                    Cod.Model.Accounting.BuildRowKey(searchFrom))
                                .ToListAsync();
