        ...
        public static async Task<HttpClientHandler> GetProxyHandler(string location, IHttpProxyFactory factory = null)
        {
            ...
            return await proxyHandlers.AddOrUpdate(location, async (l) =>
            {
                ...
                else
                {
                    ...
                    string proxyHost = await cfg.GetSettingAsStringAsync(Constant.HTTP_PROXY_HOST);
                    string proxyUsername = await cfg.GetSettingAsStringAsync(Constant.HTTP_PROXY_USERNAME);
                    string proxyPassword = await cfg.GetSettingAsStringAsync(Constant.HTTP_PROXY_PASSWORD);
                    proxy = new WebProxy
                    {
