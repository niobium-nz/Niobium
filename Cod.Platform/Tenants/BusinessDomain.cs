...
namespace Cod.Platform.Tenants
{
    public class BusinessDomain : AccountableDomain<Business>
    {
        ...
        {
            List<Interest> interests = await interestRepository.Value.GetAsync(Cod.Model.Interest.BuildPartitionKey(Guid.Parse(RowKey))).ToListAsync();
            ...
            List<Transaction> transactions = await GetTransactionsAsync(fromInclusive, toInclusive).ToListAsync();
