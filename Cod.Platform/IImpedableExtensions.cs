...
namespace Cod.Platform
{
    public static class IImpedableExtensions
    {
        ...
        public static IAsyncEnumerable<Impediment> GetImpedimentsByCategoryAsync(this IImpedable impedable, string category, CancellationToken cancellationToken = default)
        {
            ...
            return impedable.GetImpedimentsAsync(category, cancellationToken: cancellationToken);
        }
        ...
        public static async Task<Impediment> GetImpedimentAsync(this IImpedable impedable, string category, int cause, CancellationToken cancellationToken = default)
        {
            ...
            var impediments = await impedable.GetImpedimentsAsync(category, cause, cancellationToken: cancellationToken).ToArrayAsync(cancellationToken: cancellationToken);
            return impediments.SingleOrDefault();
        }

        private async static IAsyncEnumerable<Impediment> GetImpedimentsAsync(this IImpedable impedable, string category = null, int cause = 0, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            foreach (IImpedimentPolicy policy in impedable.ImpedimentPolicies)
            {
                ...
                {
                    var impediments = policy.GetImpedimentsAsync(context, cancellationToken);
                    await foreach (var item in impediments)
                    {
                        var exist = existsLockers.AddOrUpdate($"{item.PartitionKey}|{item.RowKey}", false, (k, ev) => true);
                        if (!exist)
                        {
                            yield return item;
                        }
