...
{
    internal class CloudPlatformQueue : IMessagingHub
    {
        ...
        public virtual async Task<DisposableQueueMessage> DequeueAsync(string queueName, bool createIfNotExist = true, CancellationToken cancellationToken = default)
        {
            ...
            Response<Azure.Storage.Queues.Models.QueueMessage> msg = await q.ReceiveMessageAsync(cancellationToken: cancellationToken);
        ...
        public virtual async Task<IEnumerable<DisposableQueueMessage>> DequeueAsync(string queueName, int limit, bool createIfNotExist = true, CancellationToken cancellationToken = default)
        {
            ...
            QueueClient q = await GetQueueAsync(queueName, createIfNotExist, cancellationToken);
            Response<Azure.Storage.Queues.Models.QueueMessage[]> msgs = await q.ReceiveMessagesAsync(maxMessages: limit <= 0 ? null : limit, cancellationToken: cancellationToken);
            if (msgs.Value.Any())
            {
                foreach (Azure.Storage.Queues.Models.QueueMessage msg in msgs.Value)
                {
        ...
        {
            QueueClient q = await GetQueueAsync(queueName, createIfNotExist, cancellationToken);
            Response<Azure.Storage.Queues.Models.PeekedMessage[]> msgs = await q.PeekMessagesAsync(maxMessages: limit, cancellationToken: cancellationToken);
